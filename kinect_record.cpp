	...
	float(&fAngelUsing)[ANGLE_NUM])  //普通的函数，用来执行线程
{
	...
	while (1)
	{
		if (k4a_device_get_capture(dev, &sensor_capture, K4A_WAIT_INFINITE) == K4A_WAIT_RESULT_SUCCEEDED)
		{
			...
			{
				capTidFlag = true;
				capTid = thread(&Kinct::onePicture, this, i, sensor_capture, ref(fAngelUsing), ref(tracker));
			}
...
int Kinct::capThread()
{
	...
	for (uint i = 0; i < uintNum; ++i)
	{
		tids[i] = thread(&Kinct::cap, this, ref(dev[i]), ref(colorFrame[i]), i, ref(sensorCalibration[i]), ref(fAngelUsing));
	}
	return 0;
}

int Kinct::getAngle(float (&fAngle)[ANGLE_NUM])
{
